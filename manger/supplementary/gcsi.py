...
gdsc_cell_lines = pd.read_csv("gdsc_cell_lines_metadata.csv")[["Cell line Name", ' TCGA Classfication']]
...
gcsi_gdsc = pd.merge(gdsc_cell_lines,
                     gcsi_drug_response[["cell_line"]],
                     on="cell_line").drop_duplicates()
gcsi_ccle = pd.merge(ccle_cell_lines,
                     gcsi_drug_response[["cell_line"]],
                     on="cell_line").drop_duplicates()

# find the intersection between the two databases and the gCSI cell lines, then use only one dataset to store the tumor
# types of the common cell lines to avoid different namings
ccle_gdsc_intersection = set(gcsi_gdsc["cell_line"].to_list()).intersection(gcsi_ccle["cell_line"])
gdsc_only = set(gcsi_gdsc["cell_line"]) - ccle_gdsc_intersection
gcsi_cell_lines_meta_data = pd.concat([gcsi_ccle, gcsi_gdsc[gcsi_gdsc["cell_line"].isin(gdsc_only)]])

# some cell lines were missing, which turned out to be for punctuation variations. The below conditions were successful
# to restore most of the missing types (only (7 out of 426) cell lines remained unidentified)
missing = set(gcsi_drug_response["cell_line"]) - set(gcsi_cell_lines_meta_data["cell_line"])
...
temp_5 = {'-'.join(re.split(r'(\d+)', drug.replace("-", "")))[:-1].replace(" ", ""): drug for drug in missing}
...
for temp in [temp_1, temp_2, temp_3, temp_4, temp_4_1, temp_5, temp_6]:
    ...
    gcsi_gdsc_2 = set(temp.keys()).intersection(gdsc_cell_lines["cell_line"]) - gcsi_ccle_2
    for drug in gcsi_ccle_2:
        missing.discard(temp[drug])
    for drug in gcsi_gdsc_2:
        missing.discard(temp[drug])
    gcsi_cell_lines_meta_data = pd.concat([gcsi_cell_lines_meta_data,
